...
export function VaultHeader({ onOpenUpload, documentCount }: VaultHeaderProps) {
  return (
    /* 📱 ADAPTIVE SPACING: Scaled p-6 down to p-4 on mobile view tracks */
    <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-8 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 transition-all duration-300">
      <div className="space-y-1 sm:space-y-1.5">
        <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2 sm:gap-3">
          <span className="p-1.5 sm:p-2 bg-slate-50 border border-slate-200 rounded-xl shadow-sm">
            <FolderHeart className="h-4 sm:h-5 sm:w-5 stroke-[2.5]" />
          </span>
      ...
      {/* 📱 MOBILE FIX: Button forces full-width on mobile to provide a cleaner touch area */}
      <button 
        onClick={onOpenUpload}
        className="w-full sm:w-auto h-11 px-5 bg-[#2563EB] hover:bg-blue-700 text-white font-black text-xs uppercase tracking-wider rounded-xl flex items-center justify-center gap-2 shadow-sm transition-all duration-200 active:scale-95 shrink-0 cursor-pointer"
      >
